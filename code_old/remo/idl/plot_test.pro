...
; y range, min and max values
      ymin_top = min([reform(qatom_nist[j_nist,*]),reform(qatom_irwin),reform(qmoog_tnist)])
      ymax_top = max([reform(qatom_nist[j_nist,*]),reform(qatom_irwin),reform(qmoog_tnist)])
; custom tickmarks, logarithmic y axis
      ticks  = LogLevels([ymin_top,ymax_top])
      nticks = N_Elements(ticks)
      cgplot,    temp_nist, qatom_nist[j_nist,*], linest=linest_nist, thick=thick_nist, col=color_nist, $
...
; plot bottom panel
      cgplot,/noerase, [xmin,xmax], [0,0], linest=linest_nist, thick=thick_nist, col=color_nist, $
             ...
             /xlog, xrange=[xmin,xmax], yrange=[-1,1]*1.2, xstyle=8, $
             position=[0.15,0.15,0.90,0.32], xticklen=0.1
      cgplot,/ov,temp_nist[ ww_irwin], (qatom_irwin/qatom_nist[j_nist,*]-1.)[ ww_irwin], linest=linest_nist, thick=thick_irwin, col=color_irwin
      cgplot,/ov,temp_nist[cww_irwin], (qatom_irwin/qatom_nist[j_nist,*]-1.)[cww_irwin], linest=linest_irwin, thick=thick_irwin, col=color_irwin
      cgplot,/ov,tmoog,(qmoog/qnist_tmoog-1.),psym=cgsymcat(psym_moog),col=color_moog,syms=syms_moog
      cgplot,/ov,temp_nist,(qmoog_tnist/qatom_nist[j_nist,*]-1.), linest=linest_moog, thick=thick_moog, col=color_moog
...
cd, ps_dir
;;spawn,'for i in *'+ps_suffix+'; do epstopdf $i ; done'
