...
xmax          = 1.0e+4
cd, ps_dir
cgPS_open, psfile, encapsulated=encapsulated, landscape=landscape, font=font, default_thick=default_thick, /nomatch
...
; interpolate to reference T scale
         moog_partf_t = exp(interpol(alog(moog_partf),alog(moog_temp_scale),lnt,/spline))
...
; plot NIST partition function     
      ...
      ymin  = ymin*0.9
      ymax  = ymax*1.1      
      cgPlot, t, qatom_nist, thick=thick_nist, title=title, xtitle=xtitle, ytitle=ytitle, $
              ystyle=2, /xlog, /ylog, xr=[xmin,xmax], yr=[ymin,ymax]
; overplot Irwin data
      cgPlot,/ov,temp_irw, qatom_irw, color=col_irw, thick=thick_irw;, ps='filled square'
; overplot Irwin data, on reference temperature scale
      cgPlot,/ov,t, qatom_irw_t, color=col_irw, thick=thick, linest=2
; overplot MOOG data
      if (ntemp_moog gt 0) then begin
         cgPlot,/ov, moog_temp_scale, moog_partf, color=col_moog, thick=thick_moog, ps='filled circle'
         cgPlot,/ov, t, qatom_moog_t, color=col_moog, thick=thick_moog, linest=2
      endif
