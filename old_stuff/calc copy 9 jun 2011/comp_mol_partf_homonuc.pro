;
pro comp_mol_partf_homonuc, T, molstruct, inuc,  Qi
...
;	Jmax<=1E2                         ; low or zero Bv
        ...
        ; THIS PRESENTLY ONLY VALID FOR SPECIFIC CASE H2
        phi_nuc = J*0.+1.
        ieven = where(J mod 2 eq 0., complement=iodd, ncomplement=nodd)         
        phi_nuc[ieven] = 0.25
        if nodd gt 1 then phi_nuc[iodd] = 0.75
...
;        Qrot[k] = total((2*J+1)*exp(-hckT[k]*F)) / ms.sigma		  
        Qrot[k] = total(gel*phi_nuc*(2*J+1)*exp(-hckT[k]*(F+G+ms.Te[i]-Gzero))) 
       endfor    
