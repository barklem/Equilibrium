...
for i = 0, natom-1 do begin
  ...
  read_atom, atomdir + atomdata[i], J, E, potion
...
for i = 0, nmol-1 do begin  
  read_mol, moldir + moldata[i], molstruct
  Dmol[i] = molstruct.D
...
set_plot, 'ps'
device, file = 'molecule_part.ps', ysize=25, yoffset = 2
plotthick, 3
!p.charsize=2
!p.font=-1
!p.multi=[0,1,3]
...
for i = 0, nmol-1 do begin 
   ...
   if molid[i] eq 'H2' then oplot, T, QI87, linestyle = 5
